...
def get_events():
    ...
    return {"events": [{"timestamp": r[0], "ear": r[1], "event": r[2], "pitch": r[3], "yaw": r[4]} for r in rows]}
...
while run_detector and cap.isOpened():
    ...
    chart_disp.line_chart(history_ear[-100:])  # last 100 entries
    stats_disp.text(f"EAR: {ear:.2f} | Pitch: {pitch:.1f} | Yaw: {yaw:.1f}\n"
